...
{
    public class Problem443 : IProblem
    {
        ...
        public int Compress1(char[] chars)
        {
            ...
            var forReturnArray = new List<char>(chars.Length);
            var firstIndex = 0;
            var i = 1;
            for (; i < chars.Length; i++)
            {
                ...
                {
                    Opera1(chars, forReturnArray, i, firstIndex);
                    firstIndex = i;
                }
            }
            ...
            for (var j = 0; j < forReturnArray.Count; j++)
                chars[j] = forReturnArray[j];

            return forReturnArray.Count;
        }

        private void Opera1(char[] chars, List<char> forReturnArray, int i, int firstIndex)
        {
            forReturnArray.Add(chars[firstIndex]);
            if (i - firstIndex > 1)
                forReturnArray.AddRange((i - firstIndex).ToString());
        }
    }
}
