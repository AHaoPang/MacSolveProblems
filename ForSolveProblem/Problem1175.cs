...
{
    public class Problem1175 : IProblem
    {
        public void RunProblem()
        {
            var temp = NumPrimeArrangements(5);
            if (temp != 12) throw new Exception();

            temp = NumPrimeArrangements(25);

            temp = NumPrimeArrangements(26);
            if (temp != 973508979) throw new Exception();

            temp = NumPrimeArrangements(100);
            if (temp != 682289015) throw new Exception();
        }
